...
<div id="middle">
	...
	<table cellpadding="0" cellspacing="0" border="0" class="display"
		...
		<tbody>
		<c:forEach items="${consumableList}" var="pi" varStatus="num">
				<tr class="trClass" id="${pi.pharmacyInventoryId}_${pi.cpId.provider.userId}">
					<td>${num.count}.</td>
					<td><openmrs:formatDate date="${pi.cpId.date}" type="textbox" /></td>
					<td>${pi.cpId.drugproductId.conceptId.name.name}</td>
					<td>${pi.cpId.qnty}</td>
					<td>${pi.cpId.service.name.name}</td>
					<td>${pi.cpId.patientNames}</td>
					<td>
						<a href="consumabledispensation.htm?editId=${pi.cpId.consumabledispenseId}" style="text-decoration: none;"><img class="editConsDisp" id="editConsDisp_${pi.cpId.consumabledispenseId}" src="${pageContext.request.contextPath}/images/edit.gif" style="cursor: pointer" /></a>
					</td>
				</tr>
		</c:forEach>
		</tbody>
